...
// This is the number of days we filter the devices list
// using, in other words, devices that haven't been
// accessed for `DEVICES_LIST_DAYS_AGO_FILTER` days
// will not show up in the devices list.
const DEVICES_LIST_DAYS_AGO_FILTER: u64 = 21;
    ...
    fn get_devices(&self, config: &Config, refresh_token: &str) -> Result<Vec<GetDeviceResponse>> {
        ...
        let request = Request::get(url)
            ...
            .query(&[(
                ...
                &util::past_timestamp(DEVICES_LIST_DAYS_AGO_FILTER).to_string(),
            )]);
        Ok(self.make_request(request)?.json()?)
    }
